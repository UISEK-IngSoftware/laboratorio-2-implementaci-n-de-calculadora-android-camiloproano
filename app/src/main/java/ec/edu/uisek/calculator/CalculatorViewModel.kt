...
class CalculatorViewModel : ViewModel() {
    ...
    private val maxDigits = 15 // Añadido: Límite de caracteres permitido
    ...
    private fun enterNumber(number: String) {
        if (operator == null) {
            if (number1.length >= maxDigits) return // Evita superar el límite
            number1 += number
            state = state.copy(display = number1)
        } else {
            if (number2.length >= maxDigits) return // También limita el segundo número
            number2 += number
    ...
    private fun enterOperator(op: String) {
        // Si ya había una operación pendiente, la calculamos antes de continuar
        if (operator != null && number2.isNotBlank()) {
            performCalculation()
        }
    ...
    private fun enterDecimal() {
        ...
        if (currentNumber.length >= maxDigits) return // Evita sobrepasar el límite
